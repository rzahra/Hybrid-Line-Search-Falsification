...
class Turbo1:
    ...
    def _create_candidates(self, X, fX, length, n_training_steps, hypers, nIterations):
        ...
        with torch.no_grad(), gpytorch.settings.max_cholesky_size(self.max_cholesky_size):
            ...
            #y_cand_Old = gp.likelihood(gp(X_cand_torch)).sample(torch.Size([self.batch_size])).t().cpu().detach().numpy()
        ...
        beta = np.sqrt(0.125 * np.log(2 * nIterations + 1))
        y_cand = f_mean - beta* np.sqrt(f_var)
    ...
    def _select_candidates(self, X_cand, y_cand):
        ...
        print("It is here")
        indbest = np.argsort(y_cand)

        X_next0 = X_cand[indbest, :]
        y_sort = y_cand[indbest]
        
        X_next = deepcopy(X_next0[:self.batch_size, :])
    ...
    def optimize(self):
        ...
        while self.n_evals < self.max_evals:
            ...
            ## ZR: Changing the code to work with corner points
            half_number_init = int(self.n_init/2) 
            num_random = half_number_init 
            ...
            while self.n_evals < self.max_evals and self.length >= self.length_min:
                ...
                X_cand, y_cand, _ = self._create_candidates(
                    X, fX, length=self.length, n_training_steps=self.n_training_steps, hypers={}, nIterations = self.n_evals
                )
